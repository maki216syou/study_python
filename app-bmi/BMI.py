...
class App(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.btn1 = Button(self, text="リセット", command=self.clear)
        self.btn1.grid(row=3, column=2)

        # ボタンを押したときにself.clearメソッドを呼ぶ
        self.btn2 = Button(self, text="リセット", command=self.clear)
        self.btn2.grid(row=4, column=0, columnspan=4)
        
        # textvariableでsel.var1が変わればLabelのテキストも変わる
    ...
    def clear(self):
        ...
        #self.lab7["image"] = self.img
        self.var1.set(self.initial)
        # Entryに書いている、文字の最初[0]から最後まで消すという意味
